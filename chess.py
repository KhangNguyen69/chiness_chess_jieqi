...
def choose_a_move_lv0(current, ismemove):
    ...
    for piece in mypawn:
        ...
        for (k, l) in next:
            ...
            cur_tmp[int(k)][int(l)] = cur_tmp[i][j]
            cur_tmp[i][j] = 0
...
def human_input_a_move(current, ismemove):
    print("it is black ture, input 4 nums of a black piece row from_col to_row to_col:")
    while 1:
...
def init(current, jieqi):
    ...
    for i in range(0, 9, 2):
        # ép kiểu về int
        current[3][i] = total_chesspieces_positive[int(11 + i / 2)]
        current[6][i] = total_chesspieces_negtive[int(11 + i / 2)]
...
def moverule(current, now):
    ...
    if index == 3:  # 象
        ...
        for k, l in copy.deepcopy(next):
            # ép kiểu về int
            if current[int((i + k) / 2)][int((j + l) / 2)] != 0:
                next.remove((k, l))
    if index == 4:  # 马
        next = [(i - 2, j + 1), (i - 2, j - 1), (i - 1, j + 2), (i - 1, j - 2), (i + 2, j - 1), (i + 2, j + 1),
                (i + 1, j + 2), (i + 1, j - 2)]
        for k, l in copy.deepcopy(next):
            if k < 0 or k > 9 or l < 0 or l > 8:
                next.remove((k, l))
    ...
    if index == 5 or index == 6:
        for k, l in [(k, j) for k in range(10)] + [(i, k) for k in range(9)]:
            if (k, l) == (i, j):
                continue
            nums = 0
            for (k_try, l_try) in [(i, l_try) for l_try in range(min(j, l) + 1, max(j, l))] + [(k_try, j) for k_try in
                                                                                               range(min(i, k) + 1,
                                                                                                     max(i, k))]:
    ...
    for k, l in copy.deepcopy(next):
        # ép kiểu về int
        if (current[i][j] * current[int(k)][int(l)] > 0):
            next.remove((k, l))
